...
/*
double cal_avet_pageaccess(int num_pages, int num_trials){
	/ Get pagesize & Make arr /
	int page_size = getpagesize();
	int jump = page_size/sizeof(int);
	int *arr = (int*)calloc((num_pages*jump), sizeof(int));
	...
	/ free memory & return time /
	free(arr);
	return total/(double)(num_trials*num_pages);
}
*/
...
int main(int argc, char *argv[]){
	...
	float avet = (float)((end.tv_sec-start.tv_sec)*1000000000 + (end.tv_nsec-start.tv_nsec))/(num_trials*num_pages);
	printf("%d %f\n",num_pages, avet);
