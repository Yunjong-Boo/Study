...
//print_Land(N, M, land);
			...
			int *temp_popul = (int*)malloc(sizeof(int)*((r2-r1+1)*(c2-c1+1)));
			for(int j = r1-1; j < r2; j++){
				for(int k  = c1-1; k < c2; k++){
					temp_popul[j*M+k] = land[j*M+k].popul;
				}
			}
			...
			for(int j = r1-1; j < r2; j++){
				for(int k = c1-1; k < c2; k++){
					...
					climb_highest(land, check, temp_popul[j*M+k], j, k, M);
				}
			}
...
//print_Land(N, M, land);
			...
			for(int j = r1-1; j < r2; j++){
				for(int k = c1-1; k < c2; k++){
					...
					int scatter_num = temp_popul[j*M+k]/cnt;
